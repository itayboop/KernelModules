...
static int start_plugin(void) {
    printk(KERN_INFO "starting dummy plugin2\n");

    return 0;
}

static int stop_plugin(void) {
    printk(KERN_INFO "stopping dummy plugin2\n");

    return 0;
}

static int cleanup(void) {
    printk(KERN_INFO "cleaning up dummy plugin2\n");
...
cleanup:
    if (ret != 0) {
        printk(KERN_ERR "Failed to register dummy plugin2.\n");
        SAFE_FREE(g_plugin);
    }
...
static void __exit exit_entry(void) {
    printk(KERN_INFO "unloading dummy plugin2\n");

    unregister_plugin(g_plugin);
    printk(KERN_INFO "Dummy plugin2 unregistered successfully.\n");
    SAFE_FREE(g_plugin);
}
...
MODULE_DESCRIPTION("Dummy plugin2");
